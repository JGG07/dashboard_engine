...
// Iterar sobre las etapas en el orden deseado
        ...
        Map<String, DashboardController.AdvisorStats> estadisticasCampania = new HashMap<>();
        Map<String, DashboardController.AdvisorStats> estadisticasFuente = new HashMap<>();
...
// --- Ordenar y construir listas combinadas ---
        dealsPorCampania.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    DashboardController.AdvisorStats stats = estadisticasCampania.getOrDefault(campania, new DashboardController.AdvisorStats());
                    listaCampaniasCombinadas.add(new DashboardController.CombinedCampaign(campania, cantidadDeals,
        ...
        dealsPorFuente.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    int cantidadDeals = entry.getValue();
                    DashboardController.AdvisorStats stats = estadisticasFuente.getOrDefault(fuente, new DashboardController.AdvisorStats());
                    listaFuentesCombinadas.add(new DashboardController.CombinedFuente(fuente, cantidadDeals,
                            stats.getInteresados(), stats.getContactados(), stats.getCita(),
                            stats.getVisita(), stats.getNegociacion(), stats.getApartado(), stats.getGanado()));
                });
...
// --- Totales ---
        Consumer<List<? extends Object>> agregarTotales = (lista) -> {
            ...
            for (Object obj : lista) {
                if (obj instanceof DashboardController.CombinedCampaign c) {
                ...
                } else if (obj instanceof DashboardController.CombinedFuente f) {
            ...
            String tipo = lista.get(0) instanceof DashboardController.CombinedCampaign ? "Campaign" : "Fuente";
    ...
    public String comerical(@RequestParam(value = "source", required = false) List<String> source,
        ...
        while (true) {
            ...
            for (DealsData deal : deals.getData()) {
                ...
                addTime = deal.getStageChangeTime();
                ...
                if(deal.getStatus().equalsIgnoreCase("won")) {
                    if (deal.getWonTime() != null) {
                        ...
                        // Restar 6 horas
                        adjustedTime = wonTime.minusHours(6);
                        date = adjustedTime.toLocalDate();
                    ...
                    dateTime = LocalDateTime.parse(addTime, formatter);
        ...
        DashboardController.AdvisorStats statsCampaign;
